/**
 * explore.js — Explore page controller, user search, and hashtag filter.
 */
...
export function resetExplore() {
    ...
    if (exploreList) exploreList.innerHTML = "";
    exploreLastId = null;
    exploreDone = false;
    exploreLoading = false;
}

export async function loadExplore(append = false) {
    const exploreList   = document.getElementById("explore-list");
    const exploreLoader = document.getElementById("explore-loader");
    if (!exploreList || !exploreLoader) return;
...
export function loadExploreByTag(tag) {
    activeTag = tag;
    showPage("explore");
    ...
    if (!banner) {
        ...
        const searchWrap = exploreSection.querySelector(".search-bar-wrap");
        if (searchWrap) exploreSection.insertBefore(banner, searchWrap.nextSibling);
    }
    banner.innerHTML = `
...
export function setupExplorePage() {
