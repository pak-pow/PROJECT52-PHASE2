...
export const CATALOG_HTML = `
  ...
  <main class="catalog-main">
    ...
    <div class="filter-bar" id="filter-bar">
      ...
      <button class="filter-btn" data-category="Web Dev">Web Dev</button>
...
export function renderCatalog(quizzes, onQuizSelect) {
  ...
  // Function to render a filtered list of quizzes
  function displayQuizzes(filteredList) {
  ...
  // Bind filter button click listeners (once only)
    ...
    filterBar.querySelectorAll(".filter-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const filtered = category === "All"
          ? quizzes
          : quizzes.filter((q) => q.category.toLowerCase() === category.toLowerCase() || q.category === category);
  ...
  // Initial render (respects whatever filter is currently active, defaults to All)
  ...
  const initialFiltered = currentCategory === "All"
    ? quizzes
    : quizzes.filter((q) => q.category.toLowerCase() === currentCategory.toLowerCase() || q.category === currentCategory);

  displayQuizzes(initialFiltered);
}
